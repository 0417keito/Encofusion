...
class Block(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        exp = list(reversed(range(99999)))
        items_up = [item_t(dilation=resnet_dilation_factor**exp.pop(), **items_kwargs) if item_t.__name__ == 'ResnetItem' else item_t(**items_kwargs) for item_t in items_up]
        self.items_up = Sequential(*items_up)
...
class XUNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        all_up = []
        for net in nets:
            all_down.append(net.skip_adapter)
            all_down.append(net.items_down)
        for net in reversed(nets):
            all_up.append(net.items_up)
            all_up.append(net.skip)
        self.all_down = nn.ModuleList(all_down)
        self.all_up = nn.ModuleList(all_up)
    ...
    def forward(
        ...
        *,
        ...
        skips = []
        for i in range(0, len(self.all_down), 2):
            skip_adapter_module = self.all_down[i]
            down_module = self.all_down[i+1]
            skip = skip_adapter_module(x)
            skips.append(skip)
            x = down_module(x, features, embedding, channels)
        
        for i in range(0, len(self.all_up), 2):
            up_module = self.all_up[i]
            skip_module = self.all_up[i+1]
            x = up_module(x, features, embedding, channels)
            x = skip_module(skips.pop(), x, features)
